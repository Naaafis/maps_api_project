...
class ReactAppManager:
    ...
    def list_react_files(self, directory=None):
        ...
        if not directory: # always list contents of the src directory
            directory = os.path.join(self.get_react_app_directory(), "src") 
        else:
            if 'src' not in directory:
                directory = os.path.join(self.get_react_app_directory(), "src", directory)
            else:
